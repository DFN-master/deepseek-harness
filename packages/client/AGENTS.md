...
[docs/web-styling.md](../../docs/web-styling.md) is authoritative. Shared `--dsw-*` tokens and global sheets live in `ui-theme/src/styles/`; feature components consume semantic aliases through CSS Modules and `clsx`, with no literal colors, component library, or Tailwind. Product copy ships in every locale in [LOCALE_IDS](locale/src/locale-settings.ts) (`zh`/`en`/`pt`) through the plugin's locale dictionaries — never inline in a component; code comments are English.
...
4. Tokens only in CSS; product copy in every shipped locale's dictionary; English comments.
